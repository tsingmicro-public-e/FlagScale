...
def get_hf_model(dtype, model_path=None, config=None):
    from .modeling_hf.modeling_qwen3engram import Qwen3EngramForCausalLM

    s_time = time.time()
    if config is None:
        model = Qwen3EngramForCausalLM.from_pretrained(
        ...
        with init_empty_weights():
            model = Qwen3EngramForCausalLM._from_config(config, torch_dtype=dtype)
...
def get_mg_model(dtype, pre_process, post_process):
    ...
    s_time = time.time()
    model = model_provider(engram_builder, pre_process, post_process).to(dtype)
