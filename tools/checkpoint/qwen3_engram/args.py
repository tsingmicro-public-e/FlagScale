...
def load_args_hf2mg(args):
    ...
    ## Engram args
    args.use_engram = True
    args.engram_tokenizer_name_or_path = None
    args.engram_vocab_size = hf_args["engram_vocab_size"]
    args.max_ngram_size = hf_args["max_ngram_size"]
    args.n_embed_per_ngram = hf_args["emb_size_enngram"]
    args.n_head_per_ngram = hf_args["num_engram_heads"]
    args.engram_layer_ids = hf_args["engram_layer_ids"]
    args.engram_pad_id = hf_args["pad_id"]
    args.engram_seed = hf_args["seed"]
    args.engram_kernel_size = hf_args["kernel_size"]
    args.engram_hc_mult = hf_args["hc_mult"]

    return args


def save_args_mg2hf(args):
    from .modeling_hf.configuration_qwen3engram import Qwen3EngramConfig

    config = Qwen3EngramConfig(
        vocab_size=args.vocab_size,
        max_ngram_size=args.max_ngram_size,
        hidden_size=args.hidden_size,
        hc_mult=args.engram_hc_mult,
        emb_size_enngram=args.n_embed_per_ngram,
        ...
        num_engram_heads=args.n_head_per_ngram,
    ...
    config.architectures = ["Qwen3EngramForCausalLM"]
    auto_map = dict()
    auto_map["AutoConfig"] = "configuration_qwen3engram.Qwen3EngramConfig"
    auto_map["AutoModelForCausalLM"] = "modeling_qwen3engram.Qwen3EngramForCausalLM"
