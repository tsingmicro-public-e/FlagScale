...
class DeepseekV3MoE(nn.Module):
    ...
    def moe_infer(self, x, topk_ids, topk_weight):
        ...
        if self.ep_size > 1:
            ...
            dist.all_to_all(
                list(gathered_tokens.split(input_split_sizes)), list(new_x.split(output_splits))
            )
...
class DeepseekV3Attention(nn.Module):
    ...
    def __init__(self, config: DeepseekV3Config, layer_idx: int | None = None):
        super().__init__()
        self.config = config
...
ATTENTION_CLASSES = {"eager": DeepseekV3Attention, "flash_attention_2": DeepseekV3FlashAttention2}
