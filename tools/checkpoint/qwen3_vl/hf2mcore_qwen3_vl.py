# Copied from https://github.com/alibaba/Pai-Megatron-Patch/blob/8949a6647cbf6b39837ad3dd911fa4aa0726895b/toolkits/model_checkpoints_convertor/qwen/hf2mcore_qwen2.5_vl.py. Below is the original copyright:
...
from flagscale.train.megatron.train_qwen3_vl import add_multimodal_extra_args, model_provider
...
def add_model_args(parser):
    ...
    parser.add_argument("--target-num-layers-per-virtual-pipeline-stage", type=int, default=None)

    parser.add_argument("--target-decoder-first-pipeline-num-layers", type=int, default=None)
...
def load_megatron_model(args):
    ...
    # pre_qkv_dim = args.hidden_size // args.num_attention_heads
    ...
    if args.tensor_model_parallel_size == 1 and args.pipeline_model_parallel_size == 1:
        ...
        for k, v in mid_state.items():
            if "extra_state" in k:
                ...
                viewed = [x.view(group_per_split, -1, pre_qkv_dim, args.hidden_size) for x in v]
    ...
    if len(vision_state_dicts) > 0:
        ...
        missing = list(
            filter(lambda k: "extra_state" not in k and not k.startswith("vision_model"), _missing)
        )
        unexpected = list(
            filter(
                lambda k: "extra_state" not in k and not k.startswith("vision_model"), _unexpected
            )
        )
...
def convert_checkpoint_from_megatron_to_transformers(mgmodel, hfmodel, args):
    ...
    for hfblock, mgblock in zip(hfvision.blocks, mgvision.decoder.layers):
        ...
        copied_numel += safe_copy(
            mgblock.self_attention.linear_qkv.layer_norm_bias, hfblock.norm1.bias
        )
        ...
        converted_bias = (
            mgblock.self_attention.linear_qkv.bias.view(vision_num_query_groups, 3, -1)
            .transpose(0, 1)
            .reshape(-1)
            .contiguous()
        )
        ...
        copied_numel += safe_copy(
            mgblock.self_attention.linear_proj.weight, hfblock.attn.proj.weight
        )
        copied_numel += safe_copy(mgblock.self_attention.linear_proj.bias, hfblock.attn.proj.bias)
        # mlp --> mlp
        copied_numel += safe_copy(mgblock.mlp.linear_fc1.weight, hfblock.mlp.linear_fc1.weight)
        copied_numel += safe_copy(mgblock.mlp.linear_fc1.bias, hfblock.mlp.linear_fc1.bias)
        copied_numel += safe_copy(mgblock.mlp.linear_fc2.weight, hfblock.mlp.linear_fc2.weight)
        copied_numel += safe_copy(mgblock.mlp.linear_fc2.bias, hfblock.mlp.linear_fc2.bias)

    # 2. vision projector
    hfprojector = hfvision.merger
    mgprojector = mgvision.projection
    copied_numel += safe_copy(mgvision.decoder.final_layernorm.weight, hfprojector.norm.weight)
    copied_numel += safe_copy(mgvision.decoder.final_layernorm.bias, hfprojector.norm.bias)
    copied_numel += safe_copy(mgprojector.encoder.linear_fc1.weight, hfprojector.linear_fc1.weight)
    copied_numel += safe_copy(mgprojector.encoder.linear_fc1.bias, hfprojector.linear_fc1.bias)
    copied_numel += safe_copy(mgprojector.encoder.linear_fc2.weight, hfprojector.linear_fc2.weight)
    copied_numel += safe_copy(mgprojector.encoder.linear_fc2.bias, hfprojector.linear_fc2.bias)

    # 2.1 vision deepstack
    hfdeepstack_merger_list = hfvision.deepstack_merger_list
    mgdeepstack_merger_list = mgvision.decoder.deepstack_merger_list
    mgdeepstack_norm_list = mgvision.decoder.deepstack_norm_list
    for hfdeepstack_merger, mgdeepstack_merger, mgdeepstack_norm in zip(
        hfdeepstack_merger_list, mgdeepstack_merger_list, mgdeepstack_norm_list
    ):
        copied_numel += safe_copy(mgdeepstack_norm.weight, hfdeepstack_merger.norm.weight)
        copied_numel += safe_copy(mgdeepstack_norm.bias, hfdeepstack_merger.norm.bias)
        copied_numel += safe_copy(
            mgdeepstack_merger.encoder.linear_fc1.weight, hfdeepstack_merger.linear_fc1.weight
        )
        copied_numel += safe_copy(
            mgdeepstack_merger.encoder.linear_fc1.bias, hfdeepstack_merger.linear_fc1.bias
        )
        copied_numel += safe_copy(
            mgdeepstack_merger.encoder.linear_fc2.weight, hfdeepstack_merger.linear_fc2.weight
        )
        copied_numel += safe_copy(
            mgdeepstack_merger.encoder.linear_fc2.bias, hfdeepstack_merger.linear_fc2.bias
        )

    n_params = sum(
        [
            t.numel()
            for k, t in mgvision.state_dict().items()
    ...
    for mglayer, hflayer in zip(mgllm.decoder.layers, hfllm.layers):
        ...
        qkv_weight = mglayer.self_attention.linear_qkv.weight.view(
            num_query_groups, -1, pre_qkv_dim, hidden_size
        )
        q_weight, k_weight, v_weight = torch.split(
            qkv_weight, split_size_or_sections=[value_num_per_group, 1, 1], dim=1
        )
        copied_numel += safe_copy(
            q_weight.reshape(-1, hidden_size), hflayer.self_attn.q_proj.weight
        )
        copied_numel += safe_copy(
            k_weight.reshape(-1, hidden_size), hflayer.self_attn.k_proj.weight
        )
        copied_numel += safe_copy(
            v_weight.reshape(-1, hidden_size), hflayer.self_attn.v_proj.weight
        )

        copied_numel += safe_copy(
            mglayer.self_attention.linear_proj.weight, hflayer.self_attn.o_proj.weight
        )

        # q_k norm
        copied_numel += safe_copy(
            mglayer.self_attention.q_layernorm.weight, hflayer.self_attn.q_norm.weight
        )
        copied_numel += safe_copy(
            mglayer.self_attention.k_layernorm.weight, hflayer.self_attn.k_norm.weight
        )

        gate_weight, fc1_weight = torch.split(
            mglayer.mlp.linear_fc1.weight, split_size_or_sections=args.ffn_hidden_size
        )
    ...
    n_params = sum(
        [
            t.numel()
            for k, t in hfllm.state_dict().items()
            if isinstance(t, torch.Tensor) and "extra_state" not in k
        ]
    )
    assert n_params == copied_numel, (
...
def convert_checkpoint_from_transformers_to_megatron(hfmodel, mgmodel, args):
    ...
    # pre_qkv_dim = hidden_size // num_attention_heads
    pre_qkv_dim = args.kv_channels
    ...
    for hfblock, mgblock in zip(hfvision.blocks, mgvision.decoder.layers):
        ...
        copied_numel += safe_copy(
            hfblock.norm1.bias, mgblock.self_attention.linear_qkv.layer_norm_bias
        )
        ...
        # mlp --> mlp
        fc1_weight = hfblock.mlp.linear_fc1.weight
        fc1_bias = hfblock.mlp.linear_fc1.bias
        ...
        fc2_weight = hfblock.mlp.linear_fc2.weight
        fc2_bias = hfblock.mlp.linear_fc2.bias
        copied_numel += safe_copy(fc2_weight, mgblock.mlp.linear_fc2.weight)
        copied_numel += safe_copy(fc2_bias, mgblock.mlp.linear_fc2.bias)

    # 2. vision projector
    hfprojector = hfvision.merger
    mgprojector = mgvision.projection
    copied_numel += safe_copy(hfprojector.norm.weight, mgvision.decoder.final_layernorm.weight)
    copied_numel += safe_copy(hfprojector.norm.bias, mgvision.decoder.final_layernorm.bias)

    copied_numel += safe_copy(hfprojector.linear_fc1.weight, mgprojector.encoder.linear_fc1.weight)
    copied_numel += safe_copy(hfprojector.linear_fc1.bias, mgprojector.encoder.linear_fc1.bias)
    copied_numel += safe_copy(hfprojector.linear_fc2.weight, mgprojector.encoder.linear_fc2.weight)
    copied_numel += safe_copy(hfprojector.linear_fc2.bias, mgprojector.encoder.linear_fc2.bias)
    ...
    for hfdeepstack_merger, mgdeepstack_merger, mgdeepstack_norm in zip(
        ...
        copied_numel += safe_copy(hfdeepstack_merger.norm.weight, mgdeepstack_norm.weight)
        copied_numel += safe_copy(hfdeepstack_merger.norm.bias, mgdeepstack_norm.bias)
        copied_numel += safe_copy(
            hfdeepstack_merger.linear_fc1.weight, mgdeepstack_merger.encoder.linear_fc1.weight
        )
        copied_numel += safe_copy(
            hfdeepstack_merger.linear_fc1.bias, mgdeepstack_merger.encoder.linear_fc1.bias
        )
        copied_numel += safe_copy(
            hfdeepstack_merger.linear_fc2.weight, mgdeepstack_merger.encoder.linear_fc2.weight
        )
        copied_numel += safe_copy(
    ...
    for mglayer, hflayer in zip(mgllm.decoder.layers, hfllm.layers):
        ...
        # NOTE: Make sure the qkv weight/bias layout is correct
        ...
        copied_numel += safe_copy(
            hflayer.self_attn.q_norm.weight, mglayer.self_attention.q_layernorm.weight
        )
        copied_numel += safe_copy(
            hflayer.self_attn.k_norm.weight, mglayer.self_attention.k_layernorm.weight
        )
...
def split_vision_model(mgvision, args, prefix="vision_model") -> dict[tuple, dict]:
    ...
    pre_qkv_dim = vision_hidden_size // ENCODER_NUM_ATTENTION_HEADS
...
def load_split_state_dict_to_vision_model(state_dicts, mgvision, args):
    ...
    for etp_rank in range(tp):
        ...
        for k, v in d.items():
            ...
            if not isinstance(v, torch.Tensor):
                ...
                merged_dict[k].append(v.view(group_per_split, -1, pre_qkv_dim, vision_hidden_size))
            elif "linear_qkv.bias" in k:
                merged_dict[k].append(v.view(group_per_split, -1, pre_qkv_dim))
            elif ("linear_proj" in k or "linear_fc2" in k) and "bias" not in k:
                merged_dict[k].append(v)
            elif "linear_fc1.weight" in k and ("projection" not in k and "deepstack" not in k):
                seg = vision_ffn_hidden_size // tp
    ...
    missing, unexpected = mgvision.load_state_dict(merged_dict, strict=False)
    print(f"In vision model: missing {missing}; unexpected {unexpected}")
        ...
        for tp_rank in range(args.tensor_model_parallel_size):
            ...
            for k, v in full_model.items():
                if not isinstance(v, torch.Tensor):
                    ...
                    viewed = v.view(args.num_query_groups, -1, pre_qkv_dim, args.hidden_size)
                    viewed = viewed[group_per_split * tp_rank : group_per_split * (tp_rank + 1)]
                    target_v = viewed.view(-1, args.hidden_size)
                elif "linear_qkv.bias" in k:
                    viewed = v.view(args.num_query_groups, -1, pre_qkv_dim)
                    viewed = viewed[group_per_split * tp_rank : group_per_split * (tp_rank + 1)]
                    target_v = viewed.view(-1)
                elif "linear_proj" in k or "linear_fc2" in k:
        ...
        for tp_rank in range(args.tensor_model_parallel_size):
            for pp_rank in range(args.pipeline_model_parallel_size):
                ...
                for vpp_id in range(vpp_size):
                    ...
                    for k, v in full_model.items():
                        if check_layer(layers_to_copy, k):
                            ...
                            k = re.sub(
                                r"decoder.layers.\d+",
                                f"decoder.layers.{layers_to_copy[int(res[0])]}",
                                k,
                            )
                        elif not (
                        ...
                        if "vision_model" in k:
                            ...
                            viewed = v.view(
                                args.num_query_groups, -1, pre_qkv_dim, args.hidden_size
                            )
                            viewed = viewed[
                                group_per_split * tp_rank : group_per_split * (tp_rank + 1)
                            ]
                            target_v = viewed.view(-1, args.hidden_size)
                        elif "linear_qkv.bias" in k:
                            viewed = v.view(args.num_query_groups, -1, pre_qkv_dim)
                        ...
                        if "word_embeddings" in k:
                            if pp_rank == 0 and vpp_id == 0:
                                model_part[k] = target_v
                        elif "vision_model" not in k and (
                            "output_layer" in k or "final_layernorm" in k
                        ):
                            if (
                                pp_rank == args.pipeline_model_parallel_size - 1
                                and vpp_id == vpp_size - 1
                            ):
                                model_part[k] = target_v
                        else:
                            model_part[k] = target_v
                ...
                save_state_dict(
                    ...
                    model_chunk,
                    checkpoint_name,
                    args.target_num_layers_per_virtual_pipeline_stage is not None,
                )
    else:
        raise ValueError(
            f"Got invalid TP/PP: {args.tensor_model_parallel_size}/{args.pipeline_model_parallel_size}"
        )
