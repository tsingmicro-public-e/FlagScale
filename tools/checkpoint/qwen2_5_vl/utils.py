...
def save_hfmodel(args, model, max_shard_size="10GB"):
    ...
    if USE_TRANSFORMERS_SAVE:
        ...
        shards, index = shard_checkpoint(
            output_state_dict, max_shard_size=max_shard_size, weights_name=weight_file
        )
    else:
        if not args.save_safetensors:
            logging.warning(
                "Since Transformer v4.47.0, the HF ckpt can only be saved with safetensors in the scripts."
            )
...
def build_layer_id_mapping(args):
    ...
    assert (
        args.target_decoder_first_pipeline_num_layers is None
        or args.target_num_layers_per_virtual_pipeline_stage is None
...
def save_state_dict(
    args, model_chunks, checkpoint_name, has_vpp: bool = False, save_args: bool = True
):
    """
