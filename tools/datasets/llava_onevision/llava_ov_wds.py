...
#
#    Copied from https://github.com/LLaVA-VL/LLaVA-NeXT/blob/main/llava/train/train.py
...
def preprocess_qwen(
    ...
    return dict(input_ids=input_ids, labels=targets)  # tensor(bs x seq_len)  # tensor(bs x seq_len)
...
class SupervisedWebDataset(IterableDataset):
    def __init__(
        ...
        self.dataset = wds.WebDataset(
            f"{data_path}", nodesplitter=wds.split_by_node, resampled=True, repeat=True
        ).shuffle(1000)
        rank0_print(f"Loaded WebDataset {self.dataset} from {data_path}")
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        if image_aspect_ratio == "highres":
            image = process_highres_image(
                image, self.data_args.image_processor, self.data_args.image_grid_pinpoints
            )
    ...
    def _get_item(self, sources) -> dict[str, torch.Tensor]:
        ...
        # item_id = int(str(sources[0]["id"], encoding="utf-8"))
        item_id = str(sources[0]["id"], encoding="utf-8")
        if "image" in sources[0]:
            sources[0]["image"] = str(sources[0]["image"], encoding="utf-8")
            ...
            sources[0]["video"] = str(sources[0]["video"], encoding="utf-8")
...
class LazySupervisedDataset(Dataset):
    def __init__(
        ...
        end_index = (
            (launch_index + 1) * per_launch if launch_index != launch_times - 1 else total_len
        )
    ...
    def __getitem__(self, i) -> dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                ...
                # no need to sleep
                print(
                    f"[Try other #{attempt_idx}] Failed to fetch sample {next_index}. Exception:", e
                )
                pass
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> dict:
    ...
    # train_dataset = SupervisedWebDataset(tokenizer=tokenizer, data_path=data_args.data_path, data_args=data_args)
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            if (
                ...
                or "A14B" in model_args.model_name_or_path
            ):
...
def train(attn_implementation=None):
    ...
    if model_args.version == "v0":
        if tokenizer.pad_token is None:
            smart_tokenizer_and_embedding_resize(
                special_tokens_dict=dict(pad_token="[PAD]"), tokenizer=tokenizer, model=model
            )
    ...
    with wds.ShardWriter(
        ...
        for entry in tqdm(dataloader):
            ...
            sample = {"__key__": str(global_id), "sequence.pyd": sequence}
