...
def dsv4_hybrid_attn_type(x):
    ...
    if "[" in x:
        ...
        raise ValueError(
            f"Invalid dsv4_hybrid_attn_type: {x}. Must be a list of integers or a string containing a Python list expression."
        )
...
def validate_args(args, defaults={}):
    ...
    if args.pipeline_model_parallel_layout is not None:
        ...
        assert enable_hetero is False, (
            "num_layers_per_virtual_pipeline_stage is not supported with heterogeneous parallelism for now"
        )
    ...
    if args.ffn_hidden_size is None:
        if args.swiglu:
            ...
            if multiple_of is not None:
                ...
                args.ffn_hidden_size = multiple_of * ((hidden_dim + multiple_of - 1) // multiple_of)
            else:
...
def core_transformer_config_from_args(args, config_class=None):
    ...
    if args.multi_latent_attention:
        if config_class == TransformerConfig:
            ...
            assert issubclass(config_class, MLATransformerConfig), (
                "config_class should be a subclass of MLATransformerConfig when using multi_latent_attention."
            )
