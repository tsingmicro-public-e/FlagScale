        ...
        with one_logger.get_context_manager():
            ...
            if not one_logger.store_has_key("first_logged_train_iterations_finish_time"):
                one_logger.store_set(
                    "first_logged_train_iterations_finish_time", get_timestamp_in_ms()
                )

            train_tokens = train_samples * seq_length

            e2e_metrics = {
                "first_logged_train_iterations_finish_time": one_logger.store_get(
            ...
            if log_throughput:
                if train_duration:
                    train_throughput_per_gpu = total_flops_since_current_train_start / (
                        train_duration * 10**12 * world_size
                    )
                else:
            ...
            prev_train_iterations_time_msecs_total = one_logger.store_get(
            ...
            if current_train_iteration > tracked_train_iterations:
                train_iterations_time_msecs = (
                    current_train_iterations_time_msecs_total
                    - prev_train_iterations_time_msecs_total
            ...
            prev_validation_iterations_time_msecs_total = one_logger.store_get(
            ...
            if current_validation_iteration > tracked_validation_iterations:
                validation_iterations_time_msecs = (
                    current_validation_iterations_time_msecs_total
...
def on_save_checkpoint_start(async_save):
    ...
    if one_logger:
        with one_logger.get_context_manager():
            ...
            productive_metrics = {
                "train_tflop_productive_end": float(num_floating_point_operations_so_far)
                / (10**12),
    ...
    if one_logger:
        with one_logger.get_context_manager():
            ...
            if need_update:
                ...
                if async_save:
                    save_checkpoint_sync_time_total_productive = one_logger.store_pop(
                        f"save_checkpoint_sync_time_total_productive:{prod_iteration}"
                    )
                    last_successful_save_checkpoint_sync_finish_time = one_logger.store_pop(
                        f"save_checkpoint_sync_finish_time:{prod_iteration}"
                    )
...
def on_save_checkpoint_end(save_checkpoint_duration, current_iteration, async_save):
    ...
    if one_logger:
        with one_logger.get_context_manager():
            ...
            if one_logger.store_has_key(
                "first_checkpoint_success"
            ...
            if async_save:
                ...
                one_logger.store_set(
                    f"save_checkpoint_sync_finish_time:{current_iteration}",
            ...
            save_checkpoint_sync_time_max = max(
                one_logger.store_get("save_checkpoint_sync_time_max"), save_checkpoint_duration
            )
            save_checkpoint_sync_time_min = min(
                one_logger.store_get("save_checkpoint_sync_time_min"), save_checkpoint_duration
            )
...
def track_app_tag(batch_size, world_size, seq_length):
    ...
    if one_logger:
        with one_logger.get_context_manager():
            ...
            current_app_tag = (
                f"{app_tag_run_name}_{app_tag_run_version}_{batch_size}_{world_size}_{seq_length}"
            )
