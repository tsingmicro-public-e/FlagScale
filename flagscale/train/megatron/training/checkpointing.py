...
def save_checkpoint(
    ...
    # For LEGACY checkpoints, every unique (tp_rank, ep_rank, etp_rank) shard must be written by
                ...
                if isfile(tracker_file):
                    with open(tracker_file, "r") as f:
                        ...
                        if content == str(iteration):
                            break  # Success
                print(
                    f"WandB finalization waiting for the tracker file {tracker_file} to update..."
                )
                pytime.sleep(wait_interval_seconds)
            else:
                # This block executes if the loop completes without a `break`.
                raise RuntimeError(
                    f"Timed out waiting for tracker file {tracker_file} to be updated for iteration {iteration} after {timeout_seconds} seconds."
                )
            ######### FlagScale End #########
...
def load_checkpoint(
    ...
    if ckpt_format == "torch_dist":
        ...
        convert_to_ep = (
            getattr(args, "expert_model_parallel_size", 1) != 1
            and getattr(state_dict["args"], "expert_model_parallel_size", 1) == 1
        )
        ########## FlagScale End ##########
        ...
        optim_sd_kwargs = dict(
            metadata=sharded_sd_metadata, is_loading=True, convert_to_ep=convert_to_ep
        )  ########## FlagScale Added ##########
        model_sd_kwargs = dict(metadata=sharded_sd_metadata)
