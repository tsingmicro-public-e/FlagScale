...
from flagscale.train.datasets.compute_stats import aggregate_stats, compute_episode_stats
from flagscale.train.datasets.image_writer import AsyncImageWriter, write_image
...
from flagscale.train.datasets.video_utils import (
    VideoFrame,
...
class LeRobotDatasetMetadata:
    ...
    def _flush_metadata_buffer(self) -> None:
        ...
        # Lazy import: only needed for writing datasets
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.meta = LeRobotDatasetMetadata(
            self.root, self.revision, force_cache_sync=force_cache_sync
        )
        ...
        if has_video_keys and not use_batched_encoding:
            ...
            if parallel_encoding and num_cameras > 1:
                ...
                with concurrent.futures.ProcessPoolExecutor(max_workers=num_cameras) as executor:
                    future_to_key = {
                        executor.submit(
                            _encode_video_worker, video_key, episode_index, self.root, self.fps
                        ): video_key
    ...
    def _save_episode_video(
        self, video_key: str, episode_index: int, temp_path: Path | None = None
    ) -> dict:
    ...
    def start_image_writer(self, num_processes: int = 0, num_threads: int = 4) -> None:
        ...
        self.image_writer = AsyncImageWriter(num_processes=num_processes, num_threads=num_threads)
